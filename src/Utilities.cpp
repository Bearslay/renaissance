...
namespace btils {
    ...
    double convertAngle(const double &angle, const bool &toRadians) {return toRadians ? degToRad(angle) : radToDeg(angle);}
    double normalizeDegree(const double &angle) {return normalize<double>(angle, 360.0);}
    double normalizeRadian(const double &angle) {return normalize<double>(angle, C_2PI);}
    double normalizeAngle(const double &angle, const bool &useRadians) {return useRadians ? normalizeRadian(angle) : normalizeDegree(angle);}
}
